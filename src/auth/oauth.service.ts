...
export class OAuthService {
  ...
  async findOrCreateGoogleUser(profile: GoogleProfile): Promise<User> {
    ...
    const created = await this.repo.createUser({ email: profile.email, password: null, displayName: profile.displayName });
    return this.repo.updateUser(created.id, { googleId: profile.googleId, emailVerified: new Date() });
  }
}
