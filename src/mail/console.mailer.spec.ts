...
describe('ConsoleMailer', () => {
  it('logge le code de vérification', async () => {
    ...
    await new ConsoleMailer(fakeConfig).sendVerificationCode('a@b.com', '123456');
  ...
  it('logge l\'invitation calendrier avec l\'url construite', async () => {
    const spy = vi.spyOn(console, 'log').mockImplementation(() => {});
    await new ConsoleMailer(fakeConfig).sendCalendarInvitation('guest@b.com', 'Alice', 'abc123token');
    expect(spy).toHaveBeenCalledWith(expect.stringContaining('abc123token'));
    expect(spy).toHaveBeenCalledWith(expect.stringContaining('http://localhost:3001'));
    spy.mockRestore();
  });
});
